...
**构造器本身并没有任何返回值**。所以说，“构造方法的返回类型只能是void型”是错误的

`super` 调用父类方法，一定是构造函数的第一行

自己的构造函数相互调用使用 `this(xxx)`，而且必须写在第一行
```java
public class Person {
    private String name;
    private int age;
...
有没有构造函数，构造函数是不是私有的并不影响反序列化。

但是，如果存在继承关系，如果父类没有实现 `Serializable` 接口，那么会默认调用父类的无参构造函数。如果父类实现了 `Serializable` 接口，那么父类的有没有构造函数，构造函数是不是私有的都没有影响。
...
对于 `String str = "1" + "2" `，编译器会优化为 `String str = "12"`
